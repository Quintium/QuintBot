...
// create node from a string, follows the format {move, games, [node1, node2, ...]}
Node::Node(std::string data)
{
	...
	while (data[objectStart] != ']')
	{
		...
		childNodes.push_back(Node(data.substr(objectStart, objectEnd)));
...
std::optional<Node> Node::findNode(std::vector<Move> moves)
{
	...
	// find node with next move
	for (Node node : childNodes)
	{
		if (node.move == moves[0].getNotation())
		{
			// remove the move that was made and search the next node
			moves.erase(moves.begin());
			return node.findNode(moves);
		}
	}
...
// get a weighted random opening move from the node
std::string Node::randomMove()
{
	...
	// calculate sum of child node games
	int sum = 0;
	for (Node node : childNodes)
	{
		sum += node.games;
	...
	// get move corresponding to random number (weighted with game amount)
	double subtotal = 0;
	for (Node node : childNodes)
	{
	...
	return childNodes[childNodes.size() - 1].move;
}

// opening database is the root game node
Openings::Openings(std::string data) : Node(data)
{
}

// load openings from openging database
